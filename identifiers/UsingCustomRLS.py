...
x_estimator = tools.WrapperRLS.RLS(n=3, p=2, mu=0.99)
...
for k in range(len(t)):
    ...
    x_ = x_estimator.predict(matx, p=2)
    ...
    if k != 20:
        ...
        x2 = y[0, k]
    x = np.array([x1, x2])
    x_estimator.adapt(x, matx)

    log_pred_x1[k] = x_[0]
    log_pred_x2[k] = x_[1]

print(x_estimator.w)
A_pred = np.array([[x_estimator.w[0][0], x_estimator.w[0][1]],
                   [x_estimator.w[1][0], x_estimator.w[1][1]]])
print(A_pred)
B_pred = np.array([[x_estimator.w[0][-1]],
                   [x_estimator.w[1][-1]]])
